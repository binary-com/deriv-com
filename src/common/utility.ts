...
export const getLocationHash = () => (isBrowser() && location.hash ? location.hash.slice(1).replace(/(\/)$/g, '') : '');
...
export const getLanguage = () => (isBrowser() ? localStorage.getItem('i18n') || navigator.language : null);
...
export const isIndexEven = (index: number, reverse?: boolean) => (reverse ? (index + 1) % 2 : index % 2);

export const sanitize = (input: string): string => input.replace(/[.*+?^${}()|[\]\\]/g, '');

export const sentenceCase = (input: string): string => input.charAt(0).toUpperCase() + input.slice(1);
...
export const applyDefaultValues = <Props, DefaultProps>(props: Props, default_props_object: DefaultProps) => {
    let style = '';
...
export const getDomain = () =>
    isBrowser() && window.location.hostname.includes(deriv_cookie_domain) ? deriv_cookie_domain : 'binary.sx';

export const getLocalizedUrl = (path: string, is_index: boolean, to: string) => `/${path}${is_index ? `/` : to}`;
...
export const getLiveChatStorage = () => (isBrowser() ? localStorage.getItem('live_chat_redirection') : null);
...
export const replaceLocale = (url: string): string => {
    ...
    if (!excluded_paths.some(path => url.includes(path))) {
        domains.forEach(domain => {
            if (url.includes(domain) && url.includes('zh_tw')) checked_locale = url.replace(/(zh_tw)/g, 'zh-tw');
            if (url.includes(domain) && url.includes('zh_cn')) checked_locale = url.replace(/(zh_cn)/g, 'zh-cn');
        });
    }
...
export const matchHashInURL = (hash: string) => isBrowser() && location.hash.replace('#', '') === hash;
