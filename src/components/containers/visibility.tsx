...
export const getCountryRule = () => {
    ...
    const [is_eu, setEu] = useState(false)
    const [is_uk, setUk] = useState(false)
    const [is_non_uk, setNonUk] = useState(false)
    const [is_non_eu, setNonEu] = useState(false)
    const [is_uk_eu, setUkEu] = useState(false)
    const [is_row, setRow] = useState(false)
    ...
    useLayoutEffect(() => {
        ...
        const eu = is_eu_residence || (!residence && is_eu_country) || is_eu_domain
        const uk = is_uk_residence || (!residence && is_uk_country) || is_uk_domain
        const non_uk = !uk
        const non_eu = !eu
        const uk_eu = !(!eu && !uk)
        const row = !uk_eu
