    ...
    mainEntity: [
        ...
        },
        {
            ...
            acceptedAnswer: {
                '@type': 'Answer',
                text: localize(
                    `_t_<p>A Reset Call/Reset Put option provides the opportunity for the contract's entry price (also known as the strike price) to be reset automatically at the midpoint of the trade duration if the underlying asset's price moves favorably with your market prediction.</p><p>The reset spot price is set at the precise midpoint of the total trade duration. For example, if you enter a 10-hour Reset Call option, the reset would occur at the 5-hour point, and the price at that exact 5-hour mark becomes the reset spot.</p><p>If the reset spot is beneficial based on your market prediction, it replaces your original entry price (or strike price). For it to be beneficial, the reset spot needs to be:</p><ul><li>lower than the entry price for a Reset Call contract, or</li><li>higher than the entry price for a Reset Put contract.</li></ul> <p>Your potential payout will then depend on whether the underlying asset's exit price is higher for the reset spot price (for Reset Call) or lower than the reset spot price (for Reset Put).</p> <p>If the reset spot price is less favorable, your original contract terms remain in effect (which means, it is the contract's original entry price that is considered). The reset is automatic, but whether it is applied depends on which price (original or reset) is more optimal for a potential payout.</p>_t_`,
                ),
            },
        },
        {
            ...
            acceptedAnswer: {
                '@type': 'Answer',
                text: localize(
                    `_t_<p>No, early exits are not available for Reset Call/Reset Put contracts.</p>
                    <p>Please note that the availability of early exit from your trading position depends on the specific trade type, or it may only be available within a certain timeframe.</p>_t_`,
                ),
            },
        },
        {
        ...
        },
        {
            ...
            acceptedAnswer: {
                '@type': 'Answer',
                text: localize(
                    `_t_<p>Reset Call/Reset Put contracts offer the ability to reset the strike price at a potentially more advantageous level. In this contract, your market prediction is your contract's exit price against the reset price (instead of the entry price). Reset contract types also typically offer offer a lower fixed payout percentage compared to Rise/Fall contracts.</p>
                   <p> Meanwhile, Rise/Fall contracts consider the contract's exit price against the contract's entry price, based on your market prediction.</p>
