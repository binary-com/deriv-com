...
import { financialItemLists, optionItemDefault, syntheticItemLists } from '../common/_underlying-data';
import {
    BreadCrumbContainer,
    CalculateButton,
    CalculatorBody,
...
import { Accordion, AccordionItem, Header, ImageWithDireciton, LocalizedLinkText, Text } from 'components/elements';
...
const PnlMarginCalculator = () => {
    ...
    const onTabClick = t => {
        ...
        if (form) {
            ...
            form.setFieldValue('optionList', sub_tab === 'Synthetic' ? syntheticItemLists : financialItemLists);
        }
    };
    ...
    React.useEffect(() => {
        if (form?.values.pointValue) {
            form.setFieldValue('stopLossPips', getPnlMarginCommon(formik_ref.current.values, 'getStopLossPip'));
            form.setFieldValue('takeProfitPips', getPnlMarginCommon(formik_ref.current.values, 'getTakeProfitPip'));
                    ...
                    <FormWrapper>
                        <Formik
                            ...
                            onSubmit={(values, { setFieldValue }) => {
                                setFieldValue('stopLossPips', getPnlMarginCommon(values, 'getStopLossPip'));
                                if (tab === 'Buy') {
                                    setFieldValue(
                                        'takeProfitLevel',
                                        getPnlMarginCommon(values, 'getTakeProfitLevelBuy')
                                    );
                                    setTakeProfitOutput(getPnlMarginCommon(values, 'getTakeProfitLevelBuy'));
                                    setFieldValue('stopLossLevel', getPnlMarginCommon(values, 'getStopLossLevelBuy'));
                                    setStopLossOutput(getPnlMarginCommon(values, 'getStopLossLevelBuy'));
                                }
                                if (tab === 'Sell') {
                                    setFieldValue('stopLossLevel', getPnlMarginCommon(values, 'getStopLossLevelSell'));
                                    setStopLossOutput(getPnlMarginCommon(values, 'getStopLossLevelSell'));
                                    setFieldValue(
                                        'takeProfitLevel',
                                        getPnlMarginCommon(values, 'getTakeProfitLevelSell')
                                    );
                                    setTakeProfitOutput(getPnlMarginCommon(values, 'getTakeProfitLevelSell'));
                                }
                                setFieldValue('takeProfitPips', getPnlMarginCommon(values, 'getTakeProfitPip'));
                                            ...
                                            <PnlCalculatorFormMobile>
                                                ...
                                                <CalculatorBody>
                                                    ...
                                                    <Flex jc='space-between'>
                                                        <Flex fd='column' width='100%' ml='3px' mr='2rem'>
                                                            <CalculatorDropdown
                                                        ...
                                                        >
                                                            {({ field }) => (
                                                                <Input
                                                                    ...
                                                                    autoComplete='off'
                                                                    error={
                                                                        touched.stopLossAmount && errors?.stopLossAmount
                                                                    }
                                                                    onBlur={handleBlur}
                                                                    data-lpignore='true'
                                                                    handleError={stopLossErrorHandler}
