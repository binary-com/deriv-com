import Loadable from '@loadable/component'

export const OurValues = Loadable(() => import('./_OurValues'))
export const OurPrinciples = Loadable(() => import('./_OurPrinciples'))
export const OurLeadership = Loadable(() => import('./_OurLeadership'))
export const AboutUsBanner = Loadable(() => import('./_AboutUsBanner'))
export const DerivNumbers = Loadable(() => import('./_DerivNumbers'))
export const ImageMarquee = Loadable(() => import('./carousel/_ImageMarquee'))
export const OurOffices = Loadable(() => import('./_OurOffices'))
