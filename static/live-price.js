...
setTimeout(() => {
    let prevTab = 'fx'
    var onTabChange = (tabId) => {
        console.log('tabId', tabId)
        document.getElementById('#' + prevTab).classList.remove('active-cards')
        document.getElementById(prevTab).classList.remove('active-tab')
        prevTab = tabId
        document.getElementById('#' + tabId).classList.add('active-cards')
    ...
    setTimeout(() => {
        tabs.map((tabId) => {
            console.log('tabs mounting', tabId)
            document.getElementById(tabId).addEventListener('click', function () {
                fetchData(tabId) // Fetch data when tab changes
                onTabChange(tabId) // Toggle tabs and cards
            })
        })
    }, 1000)
    ...
    function updateUI(data, tabId) {
        if (data) {
            const chngElements = document.querySelectorAll(`.percent-change.${tabId}`)
            const bidElements = document.querySelectorAll(`.bid.${tabId}`)
            const askElements = document.querySelectorAll(`.ask.${tabId}`)
            const sprdElements = document.querySelectorAll(`.spread.${tabId}`)
            const iconElements = document.querySelectorAll(`.content-top_change-image.${tabId}`)
            ...
            for (const key in data) {
                ...
                const change = parseFloat(currency.chng)
                if (change > 0) {
                ...
                if (index < iconElements.length) {
                    const icon = change === 0 ? 'remain' : change > 0 ? 'up' : 'down'
                    const selectedIcon =
    ...
    function fetchData(tabId) {
        const dataRef = ref(db, `eu/mkt/${tabId}`)
